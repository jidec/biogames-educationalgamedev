...
public class SerializeMaterial : SerializerExtensionBase<Material> {
    public override IEnumerable<object> Save(Material target) {
        ...
        if (target.GetInstanceID() >= 0) {
            ...
            return new object[] { false, target.shader.name, target.name, target.renderQueue, store != null ? store.GetValues(target) : null, target.shaderKeywords };
        }

    }

    StoreMaterials GetStore() {
        if (SerializeMeshRenderer.Store != null) {
            return SerializeMeshRenderer.Store;
        }
    ...
    public override object Load(object[] data, object instance) {
        ...
        if((data[5]) != null) {
            foreach (string keyword in (string[])data[5]) {
    ...
    public override bool CanBeSerialized(Type targetType, object instance) {
        ...
        if ((Shader.Find(obj.shader.name) != null && obj.GetInstanceID() < 0 && SerializeMeshRenderer.Store != null) || SaveGameManager.Instance.GetAssetId(instance as UnityEngine.Object).index != -1) {
            return true;
        }
        return false;
    }
...
[SubTypeSerializer(typeof(AudioMixerGroup))]
...
public class SerializeMeshRenderer : IComponentSerializer {
...
public class SerializeLineRenderer : IComponentSerializer {
    ...
    public class StoredInformation : SerializeMeshRenderer.StoredInformation {
